...
function postToAllLists(apiKeys, client_id, server_count, shard_id, shard_count, shards) {
    return __awaiter(this, void 0, void 0, function () {
        var currentDate, tmpListData, e_1, tmpLegacyIdsData, e_2;
                ...
                case 7:
                    Object.entries(listData).forEach(function (_a) {
                        ...
                        if (apiKeys[listname] && listData[listname].api_post) {
                            ...
                            bttps_1.post(apiPath, apiKeys[listname], sendObj, extendedLogging);
                        }
                    });
                    return [2 /*return*/];
            }
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    if (!client.user) return [3 /*break*/, 7];
                ...
                case 6:
                    if (!unchanged) {
                        if (repeatInterval > 2 && useBotblockAPI) {
                            ...
                            bttps_1.post('https://botblock.org/api/count', 'no key needed for this', buildBotblockData(apiKeys, client_id, server_count, shard_id, shard_count, shards), extendedLogging);
                            // they blacklisted botblock, so we need to do this, posting their stats manually
                            if (apiKeys['top.gg']) {
                                postToAllLists({ 'top.gg': apiKeys['top.gg'] }, client_id, server_count, shard_id, shard_count, shards);
                            }
                        }
                        else {
                            postToAllLists(apiKeys, client_id, server_count, shard_id, shard_count, shards);
                        }
                    }
                    return [3 /*break*/, 8];
                case 7:
...
function manualPost(guildCount, botID, apiKeys, shard_id, shard_count, shards) {
    var updatedApiKeys = convertLegacyIds(apiKeys);
    var client_id = botID;
    var server_count = guildCount;
    ...
    if (shard_id === 0 || (shard_id && !shards)) {
        ...
        if (shards) {
            if (shards.length !== shard_count) {
                console.error("BLAPI: Shardcount (" + shard_count + ") does not equal the length of the shards array (" + shards.length + ").");
                return;
            }
    ...
    if (useBotblockAPI) {
        bttps_1.post('https://botblock.org/api/count', 'no key needed for this', buildBotblockData(updatedApiKeys, client_id, server_count, shard_id, shard_count, shards), extendedLogging);
    }
    else {
        postToAllLists(updatedApiKeys, client_id, server_count, shard_id, shard_count, shards);
    }
}
